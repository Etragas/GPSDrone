import numpy as np
from utils import LinearGaussianPolicy, Dynamics
import scipy as sp
...
def sample_dist(state, actions):
    ...
    for trajectory in range(state.shape[0]):
        trajectory = 6

        dynamics = Dynamics()
        traj_states = state[trajectory,:,:]
        traj_actions = actions[trajectory,:,:]
...
def get_sample(traj_dist,experiment_id):
    ...
    states = np.genfromtxt('/home/elias/etragas@gmail.com/_Winter2015/CSC494/Trajectories/Traj'+str(experiment_id+1)+'state.txt', delimiter=',',dtype=np.float32)
    actions = np.genfromtxt('/home/elias/etragas@gmail.com/_Winter2015/CSC494/Trajectories/Traj'+str(experiment_id+1)+'action.txt' ,dtype=np.float32)
    T,d = states.shape
    ...
    f = open('/home/elias/etragas@gmail.com/_Winter2015/CSC494/Trajectories/Traj{}pred.txt'.format(experiment_id+1),'w+')
    r = open('/home/elias/etragas@gmail.com/_Winter2015/CSC494/Trajectories/Traj{}residuals.txt'.format(experiment_id+1),'w+')
...
def backward(state, action, dynamics, eta, hyperparameters):
    ...
    traj_dist = LinearGaussianPolicy(K, k, cov, inv_cov)
